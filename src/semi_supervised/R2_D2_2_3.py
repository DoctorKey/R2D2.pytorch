...
def main(context):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) >= 300 and (epoch + 1) < args.epochs and (epoch + 1 - 300) % reprediction_epoch == 0 and args.prediction:
            LOG.info("=> Predicting pseudo-label:")
            train_dataset.gen_pseudo_label()
            pre_pseudo_label(unlabel_loader, model, D2)
...
def adjust_learning_rate(optimizer, epoch, step_in_epoch, total_steps_in_epoch):
    ...
    if epoch > stage2:
        stage3 = epoch - stage2
        if stage3 < reprediction_epoch:
            lr = lr
        elif stage3 < reprediction_epoch * 2:
            lr = lr_schema[0] * ngpu
        elif stage3 < reprediction_epoch * 3:
            lr = lr_schema[1] * ngpu
        else:
            lr = lr_schema[2] * ngpu
