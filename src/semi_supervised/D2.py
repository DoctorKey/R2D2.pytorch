...
def main(context):
    ...
    LOG.info("=> loading dataset '{datadir}'".format(datadir=dataset_config.get('datadir')))
    ...
    train_loader = create_two_stream_loaders(train_dataset, train_dataset.unlabeled_idxs, train_dataset.labeled_idxs, args)
...
def check_pseudo_label_dataset(loader):
    ...
    for i, (input, target, index, isnoise) in enumerate(loader):
